...
module Scopes.IScopes (Scopes(..), applyErrors_a68, applyErrors_io, processor_a68, processor_io ,string2Env, buildChildren, applyErrors, applyDirections, build, toBlock, B.Errors, B.P, S.StrategicData(..)) where
...
import qualified Scopes.BlockA68 as BA
import qualified Scopes.BlockC as BC
...
-- build' a d | isDecl a = B.Decl (getDecl a) d <+> buildChildren build' a d
build' a d | isDecl a = mergeIts (foldr (B.ConsIts . (`B.Decl` d)) B.NilIts (getDecl a)) (buildChildren build' a d)
           | isUse a = B.Use (getUse a) d <+> buildChildren build' a d
           | isBlock a = B.Block (buildChildren build' a d) <+> B.NilIts
